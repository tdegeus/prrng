 ...
 *      -   The array can be stored as member, or as pointer to an external array.
 *      -   The random number generated by the pcg32 algorithm.
 *
...
private:
    D* m_data; ///< Pointer to data container (can be `m_data` or external)
    D m_container; ///< Data container (optional)
...
protected:
    ...
    void init(const S& shape, const T& initstate, const U& initseq)
    {
        ...
        m_container = xt::empty<typename D::value_type>(data_shape);
        m_data = &m_container;
        ...
        for (size_t i = 0; i < m_gen.size(); ++i) {
            m_cumsum.push_back(pcg32_cumsum_external(&m_container.flat(i * n), n, &m_gen[i], 0));
        }
    }
...
#ifdef PRRNG_ENABLE_ASSERT
        std::vector<size_t> s(initstate.dimension());
        std::copy(data.shape().cbegin(), data.shape().cbegin() + initstate.dimension(), s.begin());
        PRRNG_ASSERT(xt::same_shape(initstate.shape(), s));
#endif
...
public:
     ...
     */
    template <class T, class U>
    pcg32_tensor_cumsum(D& data, const T& initstate, const U& initseq, bool copy)
    {
