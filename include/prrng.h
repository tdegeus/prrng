 ...
 *
 *      -   The chunk is stored externally, see prrng::pcg32_cumsum for wrapper with storage.
...
public:
    pcg32_cumsum() = default;

protected:
    template <typename T, typename S>
    void init(double* data, size_t n, T initstate, S initseq)
    {
        ...
        bool strict = false)
    {
        ...
        if (target > m_chunk[m_size - 1]) {
            ...
            if (i + 1 < margin) {
                ...
                this->prev_chunk(get_chunk, m_size - i);
                return this->align_chunk(get_chunk, get_cumsum, target, margin);
            }
            size_t n = i + 1 - margin;
            if (n == 0) {
 ...
 */
template <class R>
class pcg32_cumsum : public detail::pcg32_cumsum {
private:
    R m_data;
...
public:
        ...
        S initseq = PRRNG_PCG32_INITSEQ)
    {
        ...
        this->init(&m_data.flat(0), m_data.size(), initstate, initseq);
    }

    /**
     * @brief Shape of the data.
     * @return const auto&
     */
