...
class Test_cumsum(unittest.TestCase):
    ...
    def test_cumsum_simple(self):
        ...
        for i in range(20):
            chunk = prrng.cumsum_chunk(chunk, da[istart + nchunk : istart + nchunk + n], n)
            istart += n
            self.assertTrue(np.allclose(chunk, a[istart : istart + nchunk]))

        for i in range(18):
            chunk = prrng.cumsum_chunk(chunk, da[istart - n : istart + 1], -n)
            istart -= n
            self.assertTrue(np.allclose(chunk, a[istart : istart + nchunk]))
    ...
    def test_example_prrng(self):
        ...
        for t in range(x.shape[1]):
            ...
            chunk = prrng.cumsum_chunk(chunk, da, shift)
